...
def on_press(key):
    global posx, posy, rot, running

    x, y = posx, posy  # Ensure x and y are always initialized

    try:
        if key.char == 'w':  # Equivalent to 'up'
            x, y = posx + 0.3 * np.cos(rot), posy + 0.3 * np.sin(rot)
        elif key.char == 's':  # Equivalent to 'down'
            x, y = posx - 0.3 * np.cos(rot), posy - 0.3 * np.sin(rot)
